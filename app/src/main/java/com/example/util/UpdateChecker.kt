...
object UpdateChecker {
    ...
    private const val DEFAULT_REPO = "SMS-FORWARDER"
    ...
    suspend fun checkForUpdates(context: Context): UpdateInfo = withContext(Dispatchers.IO) {
        ...
        try {
            val request = Request.Builder()
                .url(url)
                .header("User-Agent", "SMS-Forwarder-Update-Checker")
                .build()
    ...
    suspend fun downloadApk(
        ...
        try {
            ...
            val apkFile = File(updateDir, "sms_forwarder_update.apk")
            if (apkFile.exists()) apkFile.delete()

            val request = Request.Builder()
                .url(downloadUrl)
                .header("User-Agent", "SMS-Forwarder-APK-Downloader")
                .build()
